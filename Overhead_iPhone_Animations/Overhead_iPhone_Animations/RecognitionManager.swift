...
class RecognitionManager: ObservableObject {
    ...
    private func lookupDisplayText(for qrCode: String) -> String {
        ...
        if ["play", "back", "forward", "delete", "kaleidoscope", "long", "background"].contains(code) {
            return ""
        }
    ...
    func didLoseTarget() {
        DispatchQueue.main.async { [weak self] in
            ...
            if code == "play" {
                ...
                // Use timer like normal capture - gives time to remove QR code
                self.lostTargetTimer = Timer.scheduledTimer(withTimeInterval: self.settings.captureDelay, repeats: false) { [weak self] _ in
                    guard let self = self else { return }
                    self.delegate?.didSetBackground()
                }
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                    self.detectedData = ""
                    self.displayText = ""
